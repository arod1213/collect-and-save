...
pub fn parseOrdered(comptime T: type, args: []const [:0]const u8, comptime position: ArgPosition) !T {
...
pub fn parseNamed(comptime T: type, alloc: Allocator, args: []const [:0]const u8) !T {
    const info = @typeInfo(T);
    assert(info == .@"struct");
