...
pub fn shouldBeCollected(file_path: []const u8) bool {
    if (std.mem.containsAtLeast(u8, file_path, 1, "Application Support/Ableton")) {
        return false;
    }
    return !isStandardPreset(file_path) and !isFileCollected(file_path);
}
...
fn isFileCollected(_: []const u8) bool {
    return false;
    // return !std.mem.startsWith(u8, file_path, "./.");
}
